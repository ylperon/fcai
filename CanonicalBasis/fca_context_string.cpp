...
FCA::ContextString::ContextString(const std::vector<FCA::BitSet>& table)
    : FCA::Context(table),
    mObjName(mObjSize),
    mAttrName(mAttrSize) {}
...
FCA::ContextString& FCA::ContextString::operator =(const FCA::ContextString& cxt)
{
    if (&cxt == this)
    {
        return *this;
